...
- We can also compute the **90% *credible* interval**, which shows where the true effect can fall with a probability of 90%. 
...
To visualize the model, the most neat way is to extract a "reference grid" (*i.e.*, a theorethical dataframe with balanced data), using the `emmeans` package.
...
Our refgrid is made of equally spaced (balanced) predictor values. It also include the meedian of the posterior prediction, as well as 90% credible intervals. Now, we can plot it as follows:
